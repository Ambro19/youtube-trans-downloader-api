...
# # main.py - Complete and Fixed Version
# from pathlib import Path
# from youtube_transcript_api import YouTubeTranscriptApi
# from database import TranscriptDownload, get_db, engine, SessionLocal, create_tables
...
# # # Add these lines to your main.py (near the top after other imports)
...
# #     logger.warning(f"⚠️ Could not import payment router: {e}")
...
# ):
#     try:
#         # Get tier directly from user model (simpler approach)
#         tier = getattr(current_user, 'subscription_tier', 'free')
...
# # Add this to your main.py (after your existing /create_payment_intent/ endpoint)
# @app.post("/create_payment_intent/{plan_type}")
# async def create_payment_intent_with_plan(
#     plan_type: str,  # 'pro' or 'premium' 
#     current_user: User = Depends(get_current_user),
#     db: Session = Depends(get_db)
# ):
#     """Create payment intent for specific plan"""
#     try:
#         plans = {
#             'pro': {'amount': 9.99, 'name': 'Pro'},
#             'premium': {'amount': 19.99, 'name': 'Premium'}
...
#         plan = plans[plan_type]
#         logger.info(f"Creating payment intent for {plan['name']} plan")
...
#             currency='usd',
#             customer=stripe_customer_id,
#             metadata={'user_id': str(current_user.id), 'plan_name': plan_type},
#             automatic_payment_methods={'enabled': True}
#         )
...
#                 current_user.subscription_tier = 'premium'
#             else:
#                 current_user.subscription_tier = 'pro'  # Default
...
# # INSTEAD, add this endpoint after your existing payment endpoints:
...
#     db: Session = Depends(get_db)
# ):
#     """Create a Stripe Payment Intent for the specified plan"""
#     try:
#         # Plan details
#         plans = {
#             'pro': {'amount': 9.99, 'name': 'Pro'},
...
# #==========
# # Complete payment endpoint with database session handling
# @app.post("/create_payment_intent/")
# async def create_payment_intent(
#     current_user: User = Depends(get_current_user),
#     db: Session = Depends(get_db)
...
#     """Create a Stripe Payment Intent - defaults to Pro plan"""
#     try:
#         # For now, always create Pro plan payment intent
#         amount = 9.99
#         plan_name = "pro"
...
#             stripe_customer_id = customer.id
#             current_user.stripe_customer_id = stripe_customer_id
#             db.commit()  # Save the customer ID to database
