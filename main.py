...
from models import User, TranscriptDownload, get_db, initialize_database
...
app = FastAPI(title="YouTube Content Downloader API", version="3.1.1")
...
def root():
    return {"message": "YouTube Content Downloader API", "status": "running",
            "version": "3.1.1", "features": ["transcripts","audio","video","mobile","history","payments"],
...
# # main.py — YouTube Content Downloader API (clean drop-in)
...
#     logger.info("Timestamp middleware not loaded (optional): %s", _mw_err)

# # -----------------------------------------------------------------------------
# # Stripe configuration (safe to run without keys)
# # -----------------------------------------------------------------------------
# try:
#     import stripe
...
# except Exception as e:
#     logger.error("❌ Stripe import/config failed: %s", e)

# # Payment router (import after app exists)
# try:
...
# # -----------------------------------------------------------------------------
# # Downloads dir (robust on Windows/Linux)
# # -----------------------------------------------------------------------------
# try:
#     downloads_dir = Path.home() / "Downloads"
#     downloads_dir.mkdir(exist_ok=True)
#     test_file = downloads_dir / ".__test_write.tmp"
#     test_file.write_text("ok", encoding="utf-8")
#     test_file.unlink(missing_ok=True)
#     DOWNLOADS_DIR = downloads_dir
#     logger.info("🔥 Using user Downloads folder: %s", str(DOWNLOADS_DIR))
# except Exception as e:
#     DOWNLOADS_DIR = Path("downloads")
#     DOWNLOADS_DIR.mkdir(exist_ok=True)
#     logger.info("🔥 Using fallback downloads folder: %s (reason: %s)", str(DOWNLOADS_DIR), e)
...
# # -----------------------------------------------------------------------------
# # Transcript helpers (SRT/VTT + main function)
# # -----------------------------------------------------------------------------
...
#     proc = time.time() - start
#     rec = create_download_record(
#         db=db, user=user, kind="audio_downloads", youtube_id=vid,
#         quality=req.quality, file_format="mp3", file_size=fsize, processing_time=proc
#     )
...
#     ok, used, limit = check_usage_limit(user, "video_downloads")
#     if not ok:
#         raise HTTPException(status_code=403, detail=f"Monthly limit reached for video downloads ({used}/{limit}).")
...
#     proc = time.time() - start
#     rec = create_download_record(
#         db=db, user=user, kind="video_downloads", youtube_id=vid,
#         quality=req.quality, file_format="mp4", file_size=fsize, processing_time=proc
#     )
...
#     if is_mobile_request(request):
#         # Stream in small chunks for mobile browsers
#         def gen():
#             with open(file_path, "rb") as f:
#                 while True:
#                     chunk = f.read(8192)
...
#     @app.get("/{full_path:path}", include_in_schema=False)
#     def spa_catch_all(full_path: str):
#         # Let existing API/static routes win first; this is defined last.
#         index_file = FRONTEND_BUILD / "index.html"
#         if index_file.exists():
#             return HTMLResponse(index_file.read_text(encoding="utf-8"))
#         raise HTTPException(status_code=404, detail="Frontend not built")
