...
def get_content_type(filename: str) -> str:
    """
    Get content type for common audio/video files - no external dependencies needed
    """
    ...
    # Fallback for specific extensions we handle
...
# =============================================================================
# IMPROVED AUDIO DOWNLOAD WITH BETTER FILE HANDLING
# =============================================================================
...
def download_audio(
    ...
    """
    Download YouTube audio with enhanced file handling and corruption prevention
    """
    start_time = time.time()
    ...
    # Check connectivity and limits (existing code...)
    if not check_internet_connectivity():
        raise HTTPException(status_code=503, detail="No internet connection available.")
    
    # Check usage limits (existing code remains the same...)
    if user.usage_reset_date.month != datetime.utcnow().month:
        user.reset_monthly_usage()
        db.commit()
    ...
    if allowed != float('inf') and current_usage >= allowed:
        raise HTTPException(status_code=403, detail="Monthly limit reached for audio downloads.")
    
    # Check yt-dlp availability
    if not check_ytdlp_availability():
        raise HTTPException(status_code=500, detail="Audio download service temporarily unavailable.")
    
    # IMPROVED: Download with better file integrity
    downloads_path = str(DOWNLOADS_DIR)
    logger.info(f"Downloading audio to: {downloads_path}")
    ...
    try:
        ...
        # IMPROVED: Verify file integrity immediately after download
        if not audio_file_path or not os.path.exists(audio_file_path):
        ...
        # IMPROVED: More thorough file validation
        if file_size < 10000:  # Less than 10KB is definitely corrupted
            logger.error(f"Downloaded audio file too small ({file_size} bytes), likely corrupted")
            audio_file.unlink()
            raise Exception("Downloaded audio file is corrupted (too small)")
        
        # IMPROVED: Verify it's actually an audio file
        if not verify_audio_file_integrity(audio_file):
            logger.error("Downloaded file failed integrity check")
            audio_file.unlink()
            raise Exception("Downloaded audio file failed integrity verification")
    ...
    # IMPROVED: Better file moving with verification
    try:
        if not audio_file.samefile(final_path):
            # Copy first, then verify, then delete original
            shutil.copy2(str(audio_file), str(final_path))
            
            # Verify the copy
            if not verify_audio_file_integrity(final_path):
                final_path.unlink()
        ...
        logger.info(f"Audio file ready: {final_path} ({final_path.stat().st_size} bytes)")
        
    except Exception as e:
        logger.error(f"Error processing audio file: {e}")
        raise HTTPException(status_code=500, detail="Error processing downloaded audio file")
    
    # Update usage and create download record (existing code...)
    file_size = final_path.stat().st_size
    processing_time = time.time() - start_time
    user.increment_usage("audio_downloads")
    ...
    try:
        download_record = create_download_record_safe(
            db=db, user_id=user.id, youtube_id=video_id,
            transcript_type="audio", file_size=file_size,
            processing_time=processing_time, download_method="ytdlp",
            quality=request.quality, language="en", file_format="mp3",
            download_url=f"/download_file/{unique_filename}",  # Use improved endpoint
            expires_at=datetime.utcnow() + timedelta(hours=2),  # Extended expiry
            status="completed"
        )
        if download_record:
            db.commit()
    ...
    return {
        ...
        "filename": unique_filename,
        "original_filename": f"{video_id}_audio_{request.quality}.mp3",
        "expires_in": "2 hours",  # Extended
        "expires_at": (datetime.utcnow() + timedelta(hours=2)).isoformat(),
        "processing_time": round(processing_time, 2),
        "message": "Audio download ready - improved file integrity",
        "success": True,
        "file_verified": True  # New field to indicate verification
    }

def verify_audio_file_integrity(file_path: Path) -> bool:
    """
    Verify that the downloaded audio file is valid and not corrupted
    """
    try:
        # Check file size
        ...
        # Try to read file header to verify it's a valid audio file
        with open(file_path, 'rb') as f:
            header = f.read(10)
            
            # Check for MP3 header (ID3 tag or MP3 frame sync)
            if (header.startswith(b'ID3') or  # ID3 tag
                header.startswith(b'\xff\xfb') or  # MP3 frame sync
                header.startswith(b'\xff\xfa') or  # MP3 frame sync
                header.startswith(b'\xff\xf3') or  # MP3 frame sync
                header.startswith(b'\xff\xf2')):   # MP3 frame sync
                return True
            
            # For other audio formats, basic size check is sufficient
            return True
...
# =============================================================================
# Add this improved download endpoint to replace the existing one in main.py

@app.get("/download_file/{filename}")
async def download_file(filename: str, request: Request):
    """
    Serve downloaded files with proper headers for direct download
    IMPROVED: Better streaming, range support, and corruption prevention
    """
    ...
    # Use the simplified content type detection
    content_type = get_content_type(filename)
    
    # Get file stats
    file_size = file_path.stat().st_size
    
    # Determine content type more reliably
    ...
    if '_' in filename:
        ...
        if len(parts) >= 4:
            video_id = parts[0]
            content_type_name = parts[1]  # 'audio' or 'video'
            quality = parts[2]
            extension = filename.split('.')[-1]
            clean_filename = f"{video_id}_{content_type_name}_{quality}.{extension}"
    
    # Check for range request (important for large media files)
    range_header = request.headers.get('Range')
    
    if range_header:
        # Handle range requests for large files
        return await handle_range_request(file_path, range_header, content_type, clean_filename, file_size)
    else:
        # Regular file download with proper headers
        return FileResponse(
            path=file_path,
            ...
            headers={
                ...
                "Pragma": "no-cache",
                "Expires": "0",
                # Additional headers to prevent corruption
                "Content-Transfer-Encoding": "binary",
                "X-Content-Type-Options": "nosniff"
            }
        )

async def handle_range_request(file_path: Path, range_header: str, content_type: str, filename: str, file_size: int):
    """
    Handle HTTP range requests for large file downloads
    This prevents corruption in large audio/video files
    """
    try:
        ...
        async def file_streamer() -> Generator[bytes, None, None]:
