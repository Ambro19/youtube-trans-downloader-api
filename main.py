# main.py - Full Patched Version (SaaS-ready)
...
# --- FastAPI App Setup ---
app = FastAPI(title="YouTube Transcript Downloader SaaS API")
...
def get_subscription_status(current_user: User = Depends(get_current_user)):
    return {
        ...
        "expires_at": current_user.subscription_current_period_end.isoformat() if current_user.subscription_current_period_end else None,
...
@app.get("/")
def root():
    return {"status": "API running", "timestamp": datetime.utcnow().isoformat()}
