...
# =============================================================================
# FIXED VIDEO DOWNLOAD ENDPOINT - RESTORE ORIGINAL FUNCTIONALITY
# =============================================================================
...
def download_video(
    ...
    """
    Download YouTube video with file serving - FIXED VERSION
    """
    start_time = time.time()
    ...
    try:
        video_file_path = download_video_with_ytdlp(video_id, request.quality, output_dir=downloads_path)
        logger.info(f"Video download returned path: {video_file_path}")
        ...
        if "NameResolutionError" in str(e) or "Failed to resolve" in str(e):
            ...
            raise HTTPException(
                status_code=408,
                detail="Download timeout. The video download took too long. Please try again."
            )
        else:
            raise HTTPException(
                status_code=500,
    ...
    if not video_file_path or not os.path.exists(video_file_path):
        raise HTTPException(
            status_code=404, 
            detail="Failed to download video from this video. The video may not be available or may be restricted."
        )
    ...
    # Move/copy file to final location with verification
    try:
        logger.info(f"Moving file from {video_file} to {final_path}")
        
        if video_file.samefile(final_path):
            # File is already in the right place
            logger.info("File is already in the correct location")
        else:
            # Move/copy file to final location
        ...
        # Try copying instead
        try:
            shutil.copy2(str(video_file), str(final_path))
            if video_file.exists():
                video_file.unlink()
            logger.info(f"File copied successfully to {final_path}")
        except Exception as copy_error:
            logger.error(f"Error copying video file: {copy_error}")
            raise HTTPException(
                status_code=500,
    ...
    if not final_path.exists() or final_path.stat().st_size == 0:
        ...
        raise HTTPException(
            status_code=500,
            detail="Video file processing failed. Please try again."
        )
    ...
    logger.info(f"User {user.username} downloaded video for {video_id} ({request.quality}) - {file_size} bytes")
    ...
    return {
        ...
        "file_size_mb": round(file_size / (1024 * 1024), 2),
        "filename": unique_filename,
        "original_filename": f"{video_id}_video_{request.quality}.{file_extension}",
        "expires_in": "2 hours",
...
# =============================================================================
# SIMPLE FILE SERVING ENDPOINT - NO AIOFILES REQUIRED
# =============================================================================
...
async def download_file(filename: str):
    ...
    SIMPLE VERSION - No aiofiles dependency
    """
    ...
    if not file_path.exists():
        raise HTTPException(
            status_code=404, 
            detail="File not found or expired. Downloads expire after 2 hours."
        )
