...
# -------------------- FIXED: Account Deletion Endpoint -------------------
@app.delete("/user/delete-account", response_model=DeleteAccountResponse)
def delete_account(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    # Capture user details BEFORE any operations that might fail
    user_email = current_user.email or "unknown@unknown.com"
    username = current_user.username or "unknown_user"
    user_id = current_user.id
    
    logger.info(f"Starting account deletion for user: {username} ({user_email}) ID: {user_id}")
    
    deletion_success = True
    deletion_errors = []
    
    try:
        # Step 1: Delete subscription (with proper error handling)
        try:
            ...
            if not subscription_deleted:
                ...
                deletion_errors.append("subscription deletion incomplete")
        except Exception as e:
            logger.error(f"Subscription deletion failed for user {username}: {e}")
            deletion_errors.append(f"subscription error: {str(e)[:100]}")
        ...
        # Step 2: Delete user data (downloads, files)
        try:
            data_deleted = delete_user_data(db, current_user)
            if not data_deleted:
                logger.warning(f"Failed to properly delete user data for user {username}")
                deletion_errors.append("data deletion incomplete")
        except Exception as e:
            logger.error(f"User data deletion failed for user {username}: {e}")
            deletion_errors.append(f"data error: {str(e)[:100]}")
            try:
                db.rollback()
            except Exception:
                pass
        ...
        try:
            # Create a fresh session to avoid any rollback issues
            fresh_user = db.query(User).filter(User.id == user_id).first()
            if fresh_user:
                db.delete(fresh_user)
                db.commit()
                logger.info(f"Successfully deleted user account: {username} ({user_email}) ID: {user_id}")
            else:
                logger.warning(f"User {username} not found for final deletion - may have been deleted already")
        except Exception as e:
            logger.error(f"User account deletion failed for user {username}: {e}")
            deletion_errors.append(f"account deletion error: {str(e)[:100]}")
        ...
        if deletion_errors:
            message = f"Account deleted with some cleanup issues: {'; '.join(deletion_errors[:3])}"
            logger.warning(f"Account deletion completed with errors for {username}: {deletion_errors}")
        ...
        logger.error(f"Critical account deletion failure for user {username}: {e}")
        try:
            db.rollback()
        except Exception:
            pass
        
        # Even in case of major failure, try to provide a meaningful response
        raise HTTPException(
            status_code=500, 
            detail=f"Account deletion failed due to technical issues. Please contact support. Reference: User {username} ({user_email})"
        )
...
############# ===== A GOOD FIX TO BE TRIED FROM CHATGPT ====== #######
# With Error Message: INFO:     192.168.1.185:64505 - "GET /subscription_status/ HTTP/1.1" 404 Not Found
########################################################################

# '''
# Use this info for the Technical Architecture
# Below is a production-ready, drop-in main.py that:
# - never references Base directly (so it can’t crash if it isn’t exported),
# - calls your DB initializer safely,
# - keeps Stripe optional, adds sane CORS + security headers + a light in-memory rate limiter,
# - preserves your existing API surface (download transcript/audio/video, file delivery, auth, subscription status, history),
# includes your routers only if present (payment, batch, activity) so missing files won’t break startup.
# '''
...
# import os, re, time, socket, mimetypes, logging, threading, jwt
# from collections import defaultdict, deque
...
# )
# try:
#     from payment import router as payment_router  # optional
# except Exception:
#     payment_router = None

# try:
#     from batch import router as batch_router      # optional
# except Exception:
#     batch_router = None

# try:
#     from activity import router as activity_router  # optional
# except Exception:
#     activity_router = None

# try:
#     from auth_deps import get_current_user  # your existing dependency
# except Exception:
#     # Fallback simple dependency if the project lacks auth_deps (avoids crash during boot)
#     def get_current_user():
#         raise HTTPException(status_code=401, detail="Authentication dependency not configured")

# # ---------- safe metadata create (no hard Base reference) ----------
# try:
#     engine = getattr(_models, "engine", None)
#     Base = getattr(_models, "Base", None)
#     if engine is not None and Base is not None:
#         # if your project manages migrations via Alembic, you can remove this create_all
#         Base.metadata.create_all(bind=engine)
# except Exception as e:
#     # Don't let table creation crash the app; your initialize_database() will still run below
#     logging.getLogger("ycd").warning("Skipping Base.metadata.create_all: %s", e)

# # ---------- app ----------
# logging.basicConfig(level=logging.INFO)
# logger = logging.getLogger("ycd")
...
#     "connect-src 'self' https://api.stripe.com; frame-ancestors 'none'; base-uri 'none';"
# )
# app.add_middleware(
#     SecurityHeadersMiddleware,
#     csp=(PROD_CSP if IS_PROD else None),
#     hsts=IS_PROD,
#     apply_csp_to_api_only=True,
# )
...
#     if not IS_PROD else [FRONTEND_URL]
# )
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=[o for o in allowed_origins if o],
...
# def _sec_to_hhmmss_ms(ts: float, sep_ms: str = ".") -> str:
#     h = int(ts // 3600); m = int((ts % 3600) // 60); s = int(ts % 60); ms = int((ts - int(ts)) * 1000)
#     return f"{h:02}:{m:02}:{s:02}{sep_ms}{ms:03}"

# def segments_to_vtt(transcript) -> str:
#     lines = ["WEBVTT", "Kind: captions", "Language: en", ""]
#     for seg in transcript:
#         start = _sec_to_hhmmss_ms(seg.get("start", 0), ".")
#         end = _sec_to_hhmmss_ms(seg.get("start", 0) + seg.get("duration", 0), ".")
#         text = (seg.get("text") or "").replace("\n", " ").strip()
#         lines.append(f"{start} --> {end}"); lines.append(text); lines.append("")
#     return "\n".join(lines)
...
# # ---------- YouTube Transcript (YT API first, then yt-dlp fallback) ----------
# from youtube_transcript_api import YouTubeTranscriptApi
...
#     except Exception as e:
#         logging.getLogger("ycd").warning("YouTubeTranscriptApi failed: %s — fallback to yt-dlp", e)
#         fb = get_transcript_with_ytdlp(video_id, clean=clean)
#         if fb:
#             return fb
#         msg = str(e)
...
#                     continue
#         except Exception as e:
#             logger.warning("stale-files cleanup error: %s", e)
#         time.sleep(12 * 3600)

# @app.on_event("startup")
# async def on_startup():
#     # one place to create/upgrade DB (your initialize_database already exists)
#     try:
#         initialize_database()
#     except Exception as e:
#         logger.warning("initialize_database() failed or not implemented: %s", e)
#     threading.Thread(target=_cleanup_stale_files_loop, daemon=True).start()
#     logger.info("Environment: %s", ENVIRONMENT)
...
#         "status": "running",
#         "version": "3.4.0",
#         "features": ["transcripts","audio","video","history","payments"],
#         "downloads_path": str(DOWNLOADS_DIR),
#     }
...
#     ok, used, limit = check_usage_limit(user, usage_key)
#     if not ok:
#         label = "SRT" if req.format == 'srt' else "VTT" if req.format == 'vtt' else "clean" if req.clean_transcript else "timestamped"
#         raise HTTPException(status_code=403, detail=f"Monthly limit reached for {label} transcripts ({used}/{limit}).")
...
#     new_usage = increment_user_usage(db, user, usage_key)
#     proc = time.time() - start
#     rec = create_download_record(db=db, user=user, kind=usage_key, youtube_id=vid,
#                                  file_format=file_format, file_size=len(text), processing_time=proc)
#     return {"transcript": text, "youtube_id": vid, "clean_transcript": req.clean_transcript, "format": req.format,
#             "processing_time": round(proc, 2), "success": True, "usage_updated": new_usage, "usage_type": usage_key,
#             "download_record_id": rec.id if rec else None, "account": canonical_account(user)}
...
#     try: info = get_video_info(vid) or {}
#     except Exception as e: info, _ = {}, logger.warning("get_video_info failed: %s", e)
...
#     token = create_access_token_for_mobile(user.username)
#     direct_url = f"/download-file/audio/{final_name}?auth={token}"
#     return {"download_url": f"/files/{final_name}", "direct_download_url": direct_url, "youtube_id": vid,
#             "quality": req.quality, "file_size": fsize, "file_size_mb": round(fsize/1048576, 2), "filename": final_name,
#             "processing_time": round(proc, 2), "message": "Audio ready for download", "success": True,
#             "title": info.get("title", "Unknown Title"), "uploader": info.get("uploader", "Unknown"), "duration": info.get("duration", 0),
#             "usage_updated": new_usage, "usage_type": "audio_downloads",
#             "download_record_id": rec.id if rec else None, "account": canonical_account(user)}
...
#     direct_url = f"/download-file/video/{final_name}?auth={token}"
#     return {"download_url": f"/files/{final_name}", "direct_download_url": direct_url, "youtube_id": vid, "quality": req.quality,
#             "file_size": fsize, "file_size_mb": round(fsize/1048576, 2), "filename": final_name,
#             "processing_time": round(proc, 2), "message": "Video ready for download", "success": True,
#             "title": info.get("title", "Unknown Title"), "uploader": info.get("uploader", "Unknown"), "duration": info.get("duration", 0),
#             "usage_updated": new_usage, "usage_type": "video_downloads",
#             "download_record_id": rec.id if rec else None, "account": canonical_account(user)}
...
#     hist = [{
#         "id": d.id, "type": d.transcript_type, "video_id": d.youtube_id, "quality": d.quality or "default",
#         "file_format": d.file_format or "unknown", "file_size": d.file_size or 0,
#         "downloaded_at": d.created_at.isoformat() if d.created_at else None,
#         "processing_time": d.processing_time or 0, "status": getattr(d, "status", "completed")
#     } for d in rows]
#     return {"downloads": hist, "total_count": len(hist), "account": canonical_account(current_user)}
...
#             "usage": usage, "limits": limits, "downloads_folder": str(DOWNLOADS_DIR),
...
#     @app.get("/{full_path:path}", include_in_schema=False)
#     def spa_catch_all(full_path: str):
#         blocked = ("download_", "user/", "subscription_status", "health",
