...
# Public API (stable for main.py):
#   - get_transcript_with_ytdlp(video_id, clean=True, retries=3, wait_sec=1)
#   - download_audio_with_ytdlp(video_id, quality="192k", output_dir=None) -> str|None
#   - download_video_with_ytdlp(video_id, quality="720p", output_dir=None) -> str|None
#   - get_video_info(video_id) -> dict|None
...
import tempfile
import time
from datetime import datetime  # noqa: F401  (kept if you log timestamps elsewhere)
...
# Default download destination (main.py typically passes an explicit output_dir).
...
# Writable temp cookies target (Render’s /etc/secrets is read-only).
_COOKIES_SECRET_FILE = Path("/etc/secrets/cookies.txt")  # optional if you created this secret
_COOKIES_TMP_PATH = Path("/tmp/ycd_cookies.txt")         # yt-dlp may update the jar here

# Base yt-dlp command cache
...
def _materialize_cookies_to_tmp() -> str | None:
    ...
    Preference order:
      1) YTDLP_COOKIES_B64 (base64 of cookies.txt)
      2) YTDLP_COOKIES_FILE (path to a file in the container)
      3) /etc/secrets/cookies.txt (Render Secret File, if present)
    """
    try:
        b64 = (os.getenv("YTDLP_COOKIES_B64") or "").strip()
        if b64:
            _COOKIES_TMP_PATH.parent.mkdir(parents=True, exist_ok=True)
            # tolerate accidental surrounding quotes/newlines
            try:
                decoded = base64.b64decode(b64)
            except Exception:
                # Some shells add stray quotes — attempt to strip then decode once more
...
def _maybe_add_cookies(cmd: List[str]) -> List[str]:
    ...
    Using /tmp avoids Render read-only errors.
    """
...
def _resolve_ytdlp_base_cmd() -> List[str] | None:
    # Prefer an installed binary if present
...
def _parse_vtt(content: str, clean: bool) -> Optional[str]:
    ...
    for line in lines:
        ...
        if not line or line.startswith("WEBVTT") or line.startswith("NOTE"):
            continue
        if "-->" in line:
            # ignore timestamp parsing in clean mode
            continue
...
def download_audio_with_ytdlp(video_id: str, quality: str = "192k", output_dir: str | None = None) -> Optional[str]:
    ...
    # Probe first (same anti-bot flags).
    try:
        list_cmd = _ytdlp_cmd([
            "--restrict-filenames",
            "--list-formats",
    ...
    # locate mp3
    candidates = list(out_dir.glob(f"{video_id}_audio.*")) or list(out_dir.glob(f"{video_id}*.*"))
    if not candidates:
...
def download_video_with_ytdlp(video_id: str, quality: str = "720p", output_dir: str | None = None) -> Optional[str]:
    ...
    # Probe formats first (also trips bot checks early).
    try:
        list_cmd = _ytdlp_cmd([
            "--restrict-filenames",
            "--list-formats",
...
def _test_audio(video_id: str = "dQw4w9WgXcQ", quality: str = "192k"):
...
# # Stripe Secret Key + secret file in Render
# _COOKIES_SECRET_FILE = Path("/etc/secrets/cookies.txt")  # if you created a secret file in Render/optional: Render secret file
# _COOKIES_TMP_PATH = Path("/tmp/ycd_cookies.txt")   # writable for yt-dlp
...
# def _materialize_cookies_to_tmp() -> str | None:
#     """
#     Returns a writable cookies.txt path under /tmp if we have cookies configured.
#     Prefers YTDLP_COOKIES_B64; falls back to YTDLP_COOKIES_FILE or /etc/secrets/cookies.txt.
#     """
#     try:
#         b64 = (os.getenv("YTDLP_COOKIES_B64") or "").strip()
#         if b64:
...
#     """
#     Append '--cookies <writable tmp path>' when cookies are available.
#     yt-dlp may update the jar; /tmp avoids Render read-only errors.
#     """
#     cookies_path = _materialize_cookies_to_tmp()
#     if cookies_path:
#         cmd.extend(["--cookies", cookies_path])
