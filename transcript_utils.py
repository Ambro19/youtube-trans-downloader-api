...
# - Uses a temp dir for transcript files (no clutter, atomic cleanup)
# - Safer quality parsing (handles "720p", "1080", "best", etc.)
# - Optional cookies support via YTDLP_COOKIES (helps with age/region restrictions)
# - Random UA rotation everywhere yt-dlp is called
# - More defensive file discovery and validation
...
def _parse_height(quality: str, default: int = 720) -> int:
    """
    Extract height (e.g., 720 from '720p' or '720') with fallback.
    'best' -> a high ceiling; 'worst' -> small ceiling.
    """
    if not quality:
        return default
...
def _cleanup_temp_files(patterns: List[str], base_dir: Path):
    try:
        for pat in patterns:
...
def download_audio_with_ytdlp(video_id: str, quality: str = "medium", output_dir: str | None = None) -> str:
    ...
    try:
        ...
        candidates = [
            out_dir / f"{video_id}_audio_{quality}.mp3",
        ]
        if not candidates[0].exists():
            # broader search, pick newest/largest
            matches = list(out_dir.glob(f"{video_id}_audio_{quality}.*")) or list(out_dir.glob(f"{video_id}*.mp3")) or list(out_dir.glob("*.mp3"))
            if matches:
                candidates = [max(matches, key=lambda f: (f.stat().st_mtime, f.stat().st_size))]
    ...
    try:
        ...
        if r.returncode == 0 and r.stdout:
            try:
                ...
                # Sometimes yt-dlp prints extra lines; try to pick the last JSON object
...
def check_ytdlp_availability() -> bool:
    ...
    try:
        ...
        return False
    # Optional ffmpeg check (best-effort)
    try:
...
def estimate_file_size(video_id: str, download_type: str, quality: str) -> Optional[int]:
    try:
        ...
        if download_type == "video":
            # crude but useful ballpark (bytes/min)
            size_per_min = {
                1080: 100 * 1024 * 1024,
                720:  50 * 1024 * 1024,
                480:  25 * 1024 * 1024,
