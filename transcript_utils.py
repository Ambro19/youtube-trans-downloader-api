...
#     if p and os.path.exists(p):
#         _COOKIES_CACHE = p
#         log.info(f"▶ Using cookies file: {p}")
#         return p
...
#             tmp.flush(); tmp.close()
#             _COOKIES_CACHE = tmp.name
#             log.info(f"▶ Decoded cookies from YT_COOKIES_B64 into {tmp.name}")
#             return _COOKIES_CACHE
#         except Exception as e:
#             log.warning(f"Could not decode YT_COOKIES_B64: {e}")

#     log.info("▶ No cookies configured (YT_COOKIES_FILE / YT_COOKIES_B64).")
#     return None


# def _with_cookie_args(cmd: list[str]) -> list[str]:
#     cp = _resolve_cookies_path()
#     if cp:
#         # Always pass cookies to lower YouTube bot/consent friction
#         cmd += ["--cookies", cp]
#     # A couple of extractor hints that help on Shorts/consent walls:
#     cmd += ["--force-ipv4"]  # aligns with your env pref
#     cmd += ["--extractor-args", "youtube:player_client=android"]  # reduces consent issues
#     return cmd
