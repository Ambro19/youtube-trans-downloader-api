...
# Enhanced transcript_utils.py - FINAL CORRECTED VERSION
# 🔥 FIXES:
# - ✅ Videos now include audio (fixed yt-dlp commands)
# - ✅ Simplified format selection that works with ALL videos including old ones
# - ✅ Proper video metadata and titles
# - ✅ Enhanced audio quality
# - ✅ Better error handling and logging
# - ✅ Corrected assumptions about "Me at the zoo" video
...
# 🔥 FIXED VIDEO DOWNLOAD FUNCTION - SIMPLIFIED AND RELIABLE
def download_video_with_ytdlp(video_id: str, quality: str = "720p", output_dir: str = None) -> Optional[str]:
    """
    🔥 FIXED: Download video from YouTube using yt-dlp - SIMPLIFIED for maximum compatibility
    Works with ALL videos including old ones like "Me at the zoo"
    """
    if output_dir is None:
        output_dir = str(DEFAULT_DOWNLOADS_DIR)
    ...
    try:
        ...
        logger.info(f"🔥 Starting ENHANCED video download for {video_id}")
        logger.info(f"🔥 URL: {url}")
        logger.info(f"🔥 Output dir: {output_dir}")
        logger.info(f"🔥 Quality: {quality}")
        
        # 🔥 SIMPLIFIED: Just use best format - works with ALL videos
        format_selector = "best"
        
        # Use predictable output filename
        output_template = f"{video_id}_video_{quality}.%(ext)s"
        
        # 🔥 SIMPLIFIED COMMAND: Maximum compatibility
        cmd = [
            "yt-dlp",
            "--no-playlist",
            "--output", output_template,
            "--format", format_selector,     # 🔥 FIX: Simple, reliable format selector
        ...
        logger.info(f"🔥 Enhanced video command: {' '.join(cmd)}")
        ...
        # 🔥 ENHANCED: If download failed, try fallback formats
        if result.returncode != 0:
            logger.warning(f"⚠️ Video download failed, trying fallback formats...")
            try:
                formats_cmd = [
                ...
                if formats_result.stdout:
                    ...
                    # 🔥 FIXED: Don't truncate the format list - show everything
                    logger.info(formats_result.stdout)
                    
                    # 🔥 SIMPLIFIED: Try simple fallback formats that work with older videos
                    fallback_formats = [
                        "best[ext=mp4]",           # Best MP4
                        "worst[ext=mp4]",          # Worst MP4 (more compatible)
                        "best[ext=webm]",          # Best WebM  
                        "worst[ext=webm]",         # Worst WebM
                        "best",                    # Any best format
                        "worst"                    # Any format (last resort)
                    ]
                    
                    for i, fallback_format in enumerate(fallback_formats):
                        logger.info(f"🔥 Attempting fallback {i+1}/{len(fallback_formats)}: {fallback_format}")
                        fallback_cmd = [
                            "yt-dlp",
                            "--no-playlist", 
                            "--output", output_template,
                        ...
                        logger.info(f"🔥 Fallback {i+1} exit code: {fallback_result.returncode}")
                        
                        if fallback_result.returncode == 0:
                            logger.info(f"✅ Fallback {i+1} succeeded with format: {fallback_format}")
                            result = fallback_result
                            break
                        else:
                            logger.warning(f"⚠️ Fallback {i+1} failed: {fallback_result.stderr}")
                    
                    if result.returncode != 0:
                        logger.error(f"❌ All fallback attempts failed for {video_id}")
                        raise Exception(f"Video download failed: No compatible video formats could be downloaded.")
                    
            except Exception as debug_e:
                logger.error(f"Format analysis failed: {debug_e}")
                raise Exception(f"Video download failed: {str(debug_e)}")
        ...
        logger.info(f"✅ Enhanced video download successful: {video_file.absolute()} ({file_size} bytes)")
...
# 🔥 NEW: Test audio and video downloads
...
def test_video_download(video_id: str = "dQw4w9WgXcQ", quality: str = "720p"):
    """Test video download functionality"""
    try:
        logger.info(f"🔥 Testing video download for video: {video_id}, quality: {quality}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result and os.path.exists(result):
                file_size = os.path.getsize(result)
                logger.info(f"✅ Video download test successful: {result} ({file_size} bytes)")
                return True
            else:
        ...
        logger.error(f"❌ Video download test error: {e}")
        return False

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    
    print("🔥 Testing Enhanced Downloads Folder Setup...")
    ...
    if check_ytdlp_availability():
        ...
        print("\n🔥 Testing enhanced video download...")
        if test_video_download():
            print("✅ Video download test passed")
        else:
            print("❌ Video download test failed")
            
        # 🔥 Test with "Me at the zoo" to confirm it works
        print("\n🔥 Testing with 'Me at the zoo' video...")
        if test_video_download("jNQXAC9IVRw"):
            print("✅ 'Me at the zoo' video download works!")
        else:
            print("❌ 'Me at the zoo' video download failed")
    else:
...
# 2. Enhanced transcript_utils.py - FIXED VERSION
# 🔥 FIXES:
# - ✅ Videos now include audio (fixed yt-dlp commands)
# - ✅ Proper video metadata and titles
# - ✅ Enhanced audio quality
...
# 🔥 FIXED Video quality settings - INCLUDE AUDIO
VIDEO_FORMATS = {
    ...
    '360p': 'best[height<=360]+bestaudio/best[height<=360]'
}
